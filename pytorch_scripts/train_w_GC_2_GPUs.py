...
from strong_augmentations import create_augmentations
...
def apply_gradcam_visualization(model, val_loader, device, epoch, sequence_length=10, save_dir=None, num_samples=4, target_layer_name="layer4", show_plots=False):
    """
    ...
    """
    # For multi-GPU models, temporarily move to single GPU for GradCAM
    original_device = device
    gradcam_device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    
    # Handle DataParallel models - extract the underlying model
    if isinstance(model, nn.DataParallel):
        gradcam_model = model.module
        print("🔧 Using underlying model from DataParallel for GradCAM")
    else:
        gradcam_model = model
    
    # Move model to single GPU for GradCAM
    gradcam_model = gradcam_model.to(gradcam_device)
    gradcam_model.eval()
    ...
    if target_layer_name == "layer4":
        ...
        for name, module in gradcam_model.resnet18.named_modules():
    ...
    if target_layer is None:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            gradcam_model.to(original_device)
            model.train()
        return
    ...
    try:
        ...
        # Move data to GradCAM device
        images = images.to(gradcam_device)
        targets = targets.to(gradcam_device)
        ...
        for i in range(num_samples):
            ...
            output = gradcam_model(single_image)
        ...
        if show_plots:
            ...
            print(f"GradCAM saved as '{gradcam_path}'")
        plt.close()
        ...
        # Restore original model state
        if isinstance(model, nn.DataParallel):
            # Model is already on multiple GPUs via DataParallel
            model.train()
        else:
            # Move single model back to original device
            gradcam_model.to(original_device)
            model.train()
...
def load_checkpoint(checkpoint_path, model, optimizer=None, scheduler=None):
    ...
    # Load model state - handle DataParallel module prefix
    state_dict = checkpoint['model_state_dict']
    
    # Check if model is DataParallel but checkpoint doesn't have module prefix
    ...
    # Check if checkpoint has module prefix but model is not DataParallel
    elif not isinstance(model, nn.DataParallel) and any(key.startswith('module.') for key in state_dict.keys()):
...
def train_model(model, train_loader, val_loader, sequence_length=10, num_epochs=50, learning_rate=0.001, device='cuda', save_dir=None, gradcam_freq=1, gradcam_layer='layer4', checkpoint_freq=5, start_epoch=0, initial_train_losses=None, initial_val_losses=None, initial_best_val_loss=float('inf'), save_augmented_images=False, show_plots=False):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if is_best:
            ...
            # Save the underlying model state if using DataParallel
            best_model_state = (model.module.state_dict().copy() if isinstance(model, nn.DataParallel) 
                              else model.state_dict().copy())
            best_epoch = epoch
        ...
        if save_dir and (epoch + 1) % gradcam_freq == 0:
            ...
            apply_gradcam_visualization(model, val_loader, device, epoch + 1, sequence_length, save_dir, target_layer_name=gradcam_layer, show_plots=show_plots)
...
def evaluate_model(model, test_loader, sequence_length=10, device='cuda', save_dir=None, show_plots=False):
    ...
    if show_plots:
        ...
        print(f"Plot saved as '{predictions_plot_path}'")
    plt.close()
    ...
    if show_plots:
        ...
        print(f"Plot saved as '{evolution_plot_path}'")
    plt.close()
...
def main():
    ...
    parser.add_argument('--csv_path', type=str, 
                        default="data/combined_balanced_synchronized_dataset.csv",
    ...
    parser.add_argument('--save_dir', type=str, default='/home/anton/jetson/train_results',
    ...
    parser.add_argument('--show_plots', action='store_true',
                        help='Display plots during training and evaluation (default: False)')
    ...
    if torch.cuda.is_available():
        ...
        if args.multi_gpu and num_gpus > 1:
            ...
            use_multi_gpu = True
            print(f"🚀 Multi-GPU training enabled on {num_gpus} GPUs")
        else:
            DEVICE = torch.device('cuda:0')
            use_multi_gpu = False
            if args.multi_gpu and num_gpus == 1:
                print("⚠️  Multi-GPU requested but only 1 GPU available")
    ...
    print(f"Multi-GPU: {args.multi_gpu} (using {torch.cuda.device_count() if use_multi_gpu else 1} GPU{'s' if use_multi_gpu and torch.cuda.device_count() > 1 else ''})")
    ...
    # Use strong augmentations for training and validation
    ...
    test_loader = DataLoader(test_subset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
    ...
    if args.show_plots:
        ...
        print(f"Final plot saved as '{final_curves_path}'")
    plt.close()
    ...
    predictions, targets = evaluate_model(model, test_loader, sequence_length=args.sequence_length, device=DEVICE, save_dir=experiment_dir, show_plots=args.show_plots)
    ...
    print("\n🔄 To resume training:")
    print(f"   python train_combined_datasets.py --resume_from {os.path.join(checkpoint_dir, 'best_checkpoint.pth')} --num_epochs 100 {'--multi_gpu' if args.multi_gpu else ''}")
    if args.multi_gpu and torch.cuda.device_count() > 1:
        print(f"\n🚀 Training completed using {torch.cuda.device_count()} GPUs with DataParallel")
    print("="*60)
